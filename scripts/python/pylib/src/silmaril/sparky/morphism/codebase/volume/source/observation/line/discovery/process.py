from config.constants.morphism.codebase.volume.source.observation.line.discovery.process.command.value import VALUE as COMMAND
...
def MAIN() -> int:
    return STATUS(SUBPROCESS.run(COMMAND, cwd=SYS.argv[1]).returncode)
