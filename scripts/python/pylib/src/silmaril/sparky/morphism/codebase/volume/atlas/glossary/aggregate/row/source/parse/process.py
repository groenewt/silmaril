...
from config.constants.morphism.codebase.volume.atlas.glossary.aggregate.row.source.parse.process.command.value import VALUE as COMMAND
...
def MAIN() -> int:
    return SUBPROCESS.run(
        ...
        env=OS.environ | {ROW_SOURCE_GLOB: SYS.argv[2]},
    ).returncode
