...
from config.constants.morphism.contract.validation.schema.format.date.time.effect.applied.identity.value import VALUE as APPLIED_EFFECT
from config.constants.morphism.contract.validation.schema.format.date.time.effect.decode.failure.identity.value import VALUE as DECODE_FAILURE_EFFECT
from config.constants.morphism.contract.validation.schema.format.date.time.effect.rejected.identity.value import VALUE as REJECTED_EFFECT
...
CLASSIFIER = REGEX.compile(rb"(?P<accepted>" + DATE_TIME + rb")|(?P<decode_failure>[\x00-\xff]*[\x80-\xff][\x00-\xff]*)|(?P<rejected>[\x00-\x7f]*)")
FRAMES = {
