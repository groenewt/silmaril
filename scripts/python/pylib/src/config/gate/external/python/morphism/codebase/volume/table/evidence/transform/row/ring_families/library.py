...
from config.gate.external.python.morphism.codebase.volume.table.evidence.state.name.library import NAME
...
def STATE(state: dict) -> dict:
    return {**state, ROWS_KEY: [[IDENTIFIER(state), NAME(state), RECORD_PATH(record), RECORD_NUMBER(record), RECORD_TEXT(record)] for record in RECORDS(state)]}
