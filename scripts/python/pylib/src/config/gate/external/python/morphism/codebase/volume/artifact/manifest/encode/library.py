...
from config.gate.external.python.morphism.codebase.volume.artifact.manifest.document.library import DOCUMENT
from config.gate.external.python.morphism.codebase.volume.artifact.manifest.encode.reorder.library import REORDER
...
def PROJECT(value: Input) -> Frame:
    return FRAME(DOCUMENT(REORDER(DECODE(VALUE(value)))))
