...
def test_make_failure_keeps_raw_origin_error_and_status(tmp_path: Path) -> None:
    invalid = tmp_path / "invalid.csv"
    _write_rows(invalid, (("too", "short"),))
    artifact_root = tmp_path / "failed-artifacts"
    result = subprocess.run(
        (
            ...
            f"VOLUME_SOURCE_FILES_ROW_FILES={invalid}",
        ),
        cwd=_ROOT,
    ...
    failed = artifact_root / "source_files/aggregate/shape-validation.carrier"
    raw_error = (Path(f"{failed}.stderr")).read_bytes()
    assert result.returncode != 0
    assert raw_error
    assert result.stderr.startswith(raw_error)
