...
nothing), and the V1 hygiene shape (no punning — fixes the `frame.ttl:7` `a owl:Class, model:Product,
model:Frame` class-punning class in the algebra subtree; no class as a provenance participant). Theorem/axiom
...
**RED.** For each gate a probe that fails until the gate is wired: V0 (Turtle parse), V1 (no punning + no
class-as-provenance-participant — fails on the `frame.ttl:7` punning pattern if copied), V2 (`empty ⇒ REJECT`
...
commit → the provenance job (`ci.yml:109-119`, `fetch-depth:0`) REDs. **E20 / S20 (double-dip: validate ×2).**
